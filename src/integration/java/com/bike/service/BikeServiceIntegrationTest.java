...
public class BikeServiceIntegrationTest extends BasicServiceIntegrationTest {
  ...
  @Autowired private BikeHireRepository bikeHireRepository;
  ...
  void shouldDeleteBike() {
    ...
    Throwable exception =
        assertThrows(EntityNotFoundException.class, () -> bikeService.getById(bike.getId()));
    assertEquals(
  ...
  void shouldReturnAllHiresForBike() {
    ...
    assertThat(hiresForBike.getFirst()).usingRecursiveComparison().isEqualTo(hire);
  }
  ...
  void shouldReturnAllFutureHiresForBike() {
    ...
    assertEquals(1, hiresForBike.size());
    assertThat(hiresForBike.getFirst()).usingRecursiveComparison().isEqualTo(hire2);
  }
}
