...
public class BikeHireService {
        ...
        .orElseThrow(
            () ->
                new EntityNotFoundException(
                    "Unable to find com.bike.model.BikeHire with id " + id));
  }
  ...
  private void checkIfHireValid(BikeHire hire) {
    ...
    List<BikeHire> foundConflictingHires =
        futureHiresForBike.stream()
            .filter(
                bh ->
                    !hire.getStartDate().isAfter(bh.getEndDate())
                        && !hire.getEndDate().isBefore(bh.getStartDate()))
            .toList();
    if (!foundConflictingHires.isEmpty()) {
